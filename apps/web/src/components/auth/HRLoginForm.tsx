...
import { Button, Input, Badge } from "@futurecareer/ui";
...
export function HRLoginForm() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        router.push(result.data?.redirectTo ?? "/hr/dashboard");
      } else {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-white">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-slate-900 relative overflow-hidden items-center justify-center p-12">
        <div
          className="absolute top-0 right-0 w-full h-full bg-cover bg-center opacity-30 mix-blend-soft-light"
          style={{
            backgroundImage:
        ...
        <div className="relative z-10 max-w-lg text-white">
          ...
          <h2 className="text-5xl font-bold font-kanit mb-6 leading-tight text-white">
            ค้นหาบุคลากรที่ใช่ <br />
            ด้วยระบบอัจฉริยะ
          </h2>
          <p className="text-xl text-slate-300 leading-relaxed mb-8 font-sarabun">
            ยินดีต้อนรับ HR และ Recruiter ทุกท่าน!
            เข้าใช้งานระบบจัดการผู้สมัคร (ATS) ที่ทันสมัยที่สุด
            เพื่อสร้างทีมที่มีคุณภาพให้องค์กรของคุณ
          </p>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white/5 backdrop-blur-md p-4 rounded-xl border border-white/10">
              ...
              <p className="text-xs text-slate-400 font-sarabun">
                สร้างภาพลักษณ์ที่ดีผ่าน Company Feed
              </p>
            </div>
            <div className="bg-white/5 backdrop-blur-md p-4 rounded-xl border border-white/10">
              ...
              <p className="text-xs text-slate-400 font-sarabun">
                จัดการ Candidate ด้วย Kanban Board
              </p>
            </div>
          </div>
        </div>
        ...
        >
          <div className="text-center mb-10">
            ...
            <p className="text-muted-foreground font-sarabun">
              ป้อนอีเมลบริษัทและรหัสผ่านเพื่อจัดการประกาศงานของคุณ
            </p>
          </div>
          ...
          <div className="flex bg-slate-100 p-1 rounded-xl mb-8 border border-slate-200">
            <Link
              href="/login"
              className="flex-1 py-2 text-sm font-medium text-slate-500 hover:text-slate-900 text-center"
            >
              สำหรับผู้หางาน
            </Link>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <Button
              ...
              disabled={isPending}
            >
              {isPending ? "กำลังเข้าสู่ระบบ..." : "เข้าสู่ระบบ HR"}
            </Button>
          </form>
